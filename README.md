...
![WordPress](https://img.shields.io/badge/WordPress-6.x-blue) ![PHP](https://img.shields.io/badge/PHP-8.2-purple) ![Tailwind](https://img.shields.io/badge/Tailwind-3.4-06B6D4) ![Timber](https://img.shields.io/badge/Timber-2.x-green) ![Railway](https://img.shields.io/badge/Deploy-Railway-0B0D0E)
...
| Production Runtime | Docker (Apache + PHP 8.2)   | Official `wordpress:php8.2-apache` base image                |
...
Visit http://localhost:8888 — admin at `/wp-admin` with `admin` / `password`.
...
├── docker/
│   └── railway-entrypoint.sh     Apache port + proxy-TLS adapter
...
The repo ships a production-ready `Dockerfile` and entrypoint shim tailored for Railway.

### What the image contains

- `wordpress:php8.2-apache` base
- WordPress core
...
3. **Add a persistent volume** to the Web service, mounted at:
   ```
   /var/www/html/wp-content/uploads
   ```
   This keeps user-uploaded media across deploys.

4. **Set the Web service environment variables** (see [Environment Variables](#environment-variables)). At minimum, wire WP's DB vars to Railway's MySQL service:
   ```
...
5. **Deploy.** After first boot visit the Railway-provided URL and run the WordPress installer. Then activate:
   - Theme: **Snazzy Sprocket**
   - Plugins: **Advanced Custom Fields**, **Contact Form 7**

   Or via the Railway shell:
   ```bash
   railway run wp core install \
       --url=https://your-app.up.railway.app \
       --title='Snazzy Sprocket' \
       --admin_user=admin \
       --admin_email=you@example.com \
       --admin_password=change-me
   railway run wp theme activate snazzy-sprocket
   railway run wp plugin activate advanced-custom-fields contact-form-7
   ```
...
### What the Railway entrypoint does

`docker/railway-entrypoint.sh` runs before the stock WordPress entrypoint and:

1. Rewrites Apache's `Listen` directive to Railway's dynamic `$PORT` (falls back to 80 locally).
2. Appends a `WORDPRESS_CONFIG_EXTRA` snippet that trusts `X-Forwarded-Proto`, so WP generates `https://` URLs behind Railway's TLS-terminating edge.
3. Hands off to `docker-entrypoint.sh`, which copies WP core + our theme/plugins into `/var/www/html` on first boot and writes `wp-config.php` from env vars.
...
| `WORDPRESS_CONFIG_EXTRA`        | No       | Extra PHP appended to `wp-config.php`. The entrypoint adds the `X-Forwarded-Proto` snippet automatically; anything set here is preserved. |
| `PORT`                          | Auto     | Injected by Railway. The entrypoint wires Apache to listen on it.       |
...
docker run --rm -p 8080:80 snazzy-sprocket:prod  # Smoke test
